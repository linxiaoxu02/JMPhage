...
JMPhage is a command-line workflow for end-to-end analysis of tailed bacteriophages. Starting from assembled phage genomes, with optional paired-end sequencing reads, JMPhage connects read mapping, packaging-mechanism inference, gene prediction, functional annotation, genome visualization, protein-sharing network analysis, comparative genome visualization, intergenomic similarity analysis, and phylogenetic placement.

The pipeline is designed for small batches of cultured or isolate phages and for mixed analyses that combine newly sequenced phages with reference genomes. JMPhage uses isolated Conda environments to manage tools with incompatible dependencies, including Pharokka, PhageTerm, vConTACT2, Clinker, VIRIDIC, and GLUVAB.
...
```bash
git clone https://github.com/lllxxxxxxxxxxxxxx/JMPhage.git
cd JMPhage
pip install .
```
...
For offline servers, download `jm_db.tar.gz` manually from Zenodo, then extract it:

```bash
mkdir -p /path/to/jm_db
tar -xzf jm_db.tar.gz -C /path/to/jm_db
```
